...
#[component]
pub fn ResourceView<ViewFn, ResourceValue, ViewValue, Props, Adapter>(
...
where
    ...
    ViewFn: Send + Sync + 'static + Fn(Props) -> ViewValue,
    Props: Send + Sync + 'static,
    Adapter: Fn(ResourceValue) -> Props + Send + 'static,
{
    let fallback = move || Fallback(FallbackProps { context });

    let core = move || match resource.get() {
