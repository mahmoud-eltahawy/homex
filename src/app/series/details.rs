...
impl LazyRoute for SeriesDetailPage {
    ...
    fn view(this: Self) -> AnyView {
        ...
        let refetch = move || this.series.refetch();

        let core = move || match this.series.get() {
            None => Either::Right(Either::Right(fallback())),
            Some(Err(e)) => {
                Either::Right(Either::Left(ServerFnErrorView(ServerFnErrorViewProps {
                    e,
                    refetch,
                })))
            }
            Some(Ok(series)) => Either::Left(SeriesView(SeriesViewProps {
                series,
...
fn Selectors(
    ...
    let fallback = || view! { <p class="text-gray-400">جارٍ تحميل الحلقات...</p> };
    ...
    view! {
        <div class="mt-10">
            ...
            />
            <Suspense fallback=fallback>
                {move || match episodes.get() {
                    None => Either::Left(Either::Right(fallback())),
                    Some(Err(e)) => Either::Left(Either::Left(ServerFnErrorView(ServerFnErrorViewProps { e, refetch: move ||episodes.refetch()  }))),
                    Some(Ok(season)) => Either::Right(
                        EpisodeSelector(EpisodeSelectorProps { episodes:season.episodes.clone(), selected_episode })),
                }}
            </Suspense>
        </div>
    }
    ...
    F: Fn() + 'static,
{
    view! {
        <div class="py-8 text-center">
            <div class="text-red-400 text-sm font-bold mb-2">"حدث خطأ"</div>
